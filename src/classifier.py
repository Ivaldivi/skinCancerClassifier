...
class MelanomaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_path is None:
            raise FileNotFoundError(f"No image found for stem '{stem}' in {self.image_dir}")
        image = Image.open(img_path).convert("RGB")
        if self.transform:
            image = self.transform(image)
        label = row["midas_melanoma"]    # 0 or 1
        return image, label
...
# Stratified split so melanoma/non-melanoma ratios are similar.[web:6]
